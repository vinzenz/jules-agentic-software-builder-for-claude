...
class WorkflowEngine(EventEmitter):
    ...
    def start_workflow(self, workflow_name: str) -> str:
        ...
        try:
            ...
            # Maybe branch exists or git fail, log but continue or fail
            pass
    ...
    def run_loop(self, session_id: str):
        ...
        logging.basicConfig(
            filename=log_file, level=logging.INFO, format="%(asctime)s - %(levelname)s - %(message)s", force=True
        )
        ...
        for agent_type in execution_order:
            ...
            response = self.claude.call_agent(
                agent_type=agent_type, system_prompt=system_prompt, user_input=context_xml, model=config.model_tier
            )
            ...
            if response.success:
                ...
                for artifact in response.artifacts:
                    if artifact.type == "file":
                        # Safety: prevent overwriting outside repo? sandbox handles it.
                        fpath = Path(artifact.name)
